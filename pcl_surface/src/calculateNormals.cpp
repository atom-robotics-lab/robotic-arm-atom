...
void pointCloudCallback(const sensor_msgs::PointCloud2ConstPtr &cloud1_msg)
{
    ...
    if (num_points > 0)
    {
        ...
        rotation_matrix << normals->points[centroid_index].normal_x, normals->points[centroid_index].normal_y, normals->points[centroid_index].normal_z,
            0, 0, 0,
            0, 0, 0;
        ...
        // Create a ROS message for the quaternion
        ...
        // Create a ROS message for the centroid point cloud
...
std::cout << "quaternion:\n"
        ...
        normals_pub.publish(quaternion_msg);
    }
}

int main(int argc, char **argv)
{
    ros::init(argc, argv, "normals_node");
    ros::NodeHandle nh;
normals_pub = nh.advertise<geometry_msgs::Quaternion>("/quaternion", 1);
