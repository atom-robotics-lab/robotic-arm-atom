...
const int totalMotors = 2 ;

// ----------- Motor Parameter Variables  ----------- //
int dirPin[totalMotors] = {2, 12} ;
int stepPin[totalMotors] = {3, 13} ;
...
int homeAngle = 200 ;
float errorCorrectionAngle[2] = {10,250} ;
...
void loop() {
  ...
  for (int i = 0; i < 2; i++){
    ...
    long targetPosition = stepper[i].currentPosition() + stepsForDegrees(errorCorrectionAngle[i]);
    // stepper[0].setDirection(error/abs(error)) ;
    stepper[i].moveTo(targetPosition);
