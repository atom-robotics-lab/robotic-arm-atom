...
# When using a conda environment:  cmake -DCMAKE_INSTALL_PREFIX=/usr -DPYTHON_EXECUTABLE=$(which python) -DPYTHON_VERSION=3 -DSETUPTOOLS_DEB_LAYOUT=OFF -DEMPY_EXECUTABLE=/usr/bin/empy3 -DEMPY_SCRIPT=/usr/bin/empy3 ..
make
...
# in conda environment: catkin config -DPYTHON_EXECUTABLE=$(which python) -DPYTHON_VERSION=3 -DCMAKE_BUILD_TYPE=Release --merge-devel --blacklist trac_ik trac_ik_examples trac_ik_kinematics_plugin --extend /usr
# if you have a conventional ROS installation remove `--extend  /usr` 
catkin build
```
