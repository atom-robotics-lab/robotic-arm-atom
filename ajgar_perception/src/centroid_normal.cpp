...
#include "ajgar_perception/normalsSrv.h"
...
extern "C" {
bool calculateNormals(ajgar_perception::normalsSrv::Request &req, ajgar_perception::normalsSrv::Response &res) //const float* points1, size_t size1, const float* points2, size_t size2
{

    PointCloud::Ptr mask = req.maskValue
    cout<<"YES"<<endl;
...
{
    // Initialize ROS
    ros::init(argc, argv, "normalsSrvNode");

    // Create a ROS node handle
    ros::NodeHandle nh;

    //uncommon_points_pub = nh.advertise<sensor_msgs::PointCloud2>("/uncommon_points_topic", 1);
    ros::ServiceServer service = nh.advertiseService("normalsSrv", calculateNormals);
